...
export function ensureUUID(id: string | null | undefined, entityName?: string): string {
  ...
  // For more lenient validation, if we have an ID string that doesn't match, 
  // but already tried to format it, just return the original ID string
  // This is to prevent calendar failures
  console.warn(`[UUID Debug] Using non-standard UUID format: "${idStr}" (allowing for compatibility)`);
  return idStr;
}
