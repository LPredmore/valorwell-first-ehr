...
export const useAvailabilityEdit = (
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      if (!availabilityBlock.isException && availabilityBlock.id !== 'new') {
      ...
      if (existingException) {
        ...
        if (availabilityBlock.id !== 'new' && !availabilityBlock.isException) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // If it's a standalone one-time availability (not tied to any weekly pattern)
      if (availabilityBlock.isException && availabilityBlock.isStandalone) {
        ...
        if (deleteError) {
          console.error('Error deleting one-time availability:', deleteError);
          throw deleteError;
        }
      } else if (availabilityBlock.id === 'new') {
        // Just close the dialog, nothing to delete for a new block
        setIsDeleteDialogOpen(false);
        onClose();
        return;
      } else {
        // It's a recurring availability - we need to create or update an exception
        let existingException = null;
        let checkError = null;
        ...
        if (existingException) {
          ...
          // Add original_availability_id for recurring availability
          if (!availabilityBlock.isException) {
            insertData.original_availability_id = availabilityBlock.id;
          }
